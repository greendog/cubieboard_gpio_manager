module GpioManager
  class GPIO
    ...
    def initialize(id, port)
      @id = id
      @port = port
      @@ATTRS.each { |a| instance_variable_set("@#{a}_path", "#{@@DIR}/gpio#{id}_#{port}/#{a}") }
    end

    def to_io
      @io||=File.new(@value_path)
    end

    def chmod(mode)
      File.chmod(mode, *@@ATTRS.map { |a| instance_variable_get("@#{a}_path") })
      self
    end

    def chown(owner, group)
      File.chown(owner, group, *@@ATTRS.map { |a| instance_variable_get("@#{a}_path") })
      self
    end
    ...
    def export
      self.class.export id
      self
    end

    def unexport
      self.class.unexport id
      self
    end
    ...
    def set_input
      File.write(@direction_path, 'in')
      self
    end

    def set_output
      File.write(@direction_path, 'out')
      self
    end


    def set_output_low(v=true)
      File.write(@direction_path, v ? 'low' : 'high')
      self
    end

    def set_output_high(v=true)
      set_output_low(!v)
      self
    end

    def low?
      to_io.rewind; to_io.read.chomp == '0'
    end
    ...
    def set_active_low(v = true)
      File.write(@active_low_path, v ? '1' : '0')
      self
    end
